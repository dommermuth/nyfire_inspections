...
function get_building() {
	...
	if (isset($search_text) && $user_id) {
		...
		//echo $post_table; 
		$result = $wpdb->get_results( "SELECT * FROM ".$post_table." WHERE post_type='reports' AND post_status='publish' AND post_title LIKE '%$search_text%'" );
		if ($result){
		...
		$table = "<table id='buildings'><tr><th>Address</th><th>Date</th><th>Source</th><th>New</th><th>Clone</th><th>Edit</th><th>View</th><th>Del.</th></tr>";
		foreach($buildings_ar as $building){
			...
			if($building['src'] == "crm"){
				$tr .= "<tr><td>" . $building['title'] . "</td><td>&nbsp;</td><td>CRM</td><td>
						<a href='".$home_url."/new?address=".$building['title']."&state=".$building['state']."&city=".$building['city']."&zip=".$building['zip']."' class='btn new' data-id='".$building['id']."' data-src='".$building['src']."'>New</a>
				...
				$tr .= "<tr><td>" . $building['title'] . "</td><td>" . date_format($date,"Y/m/d H:i:s") . "</td><td><span class='red bold'>FSI</span></td>
						<td><a href='".$home_url."/new?address=".$building['title']."&state=".$building['state']."&city=".$building['city']."&zip=".$building['zip']."' class='btn new' data-id='".$building['id']."' data-src='".$building['src']."'>New</a></td>
						<td><a href='".$home_url."/clone?pid=".$building['id']."' class='btn clone' data-id='".$building['id']."' data-src='".$building['src']."'>Clone</a></td>
						<td><a href='".$home_url."/edit?pid=".$building['id']."' class='btn edit' data-id='".$building['id']."' data-src='".$building['src']."'>Edit</a></td>
						<td><a href='".$home_url."/report/pdf?pid=".$building['id']."' class='btn view' data-id='".$building['id']."' data-src='".$building['src']."' target='_blank'>PDF</a></td>
