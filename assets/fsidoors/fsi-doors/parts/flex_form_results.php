<?php
	...
	$is_repeater = get_sub_field('is_repeater');
	$repeater_name = get_sub_field('repeater_name');
	...
	if(!$is_repeater && !empty($image_field) && have_rows($image_field, $pid) ){
	...
	if( empty($is_repeater) && !empty($image_field_2) && have_rows($image_field_2, $pid) ){
